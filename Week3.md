...
---
### 11. 생성자
  + `new 연산자`에 의해 호출되어 객체의 초기화 담당
  + 생성자를 선언하지 않은 경우 컴파일 시 자동 추가
  + `생성자 오버로딩(Overloading)`
    + 객체를 생성할 때, 가변적인 형태의 외부 값으로 객체를 초기화할 필요 有
    + 매개변수의 타입, 개수, 순서가 다른 생성자 여러 개 선언
  + `this`
    + `this 참조 변수` : 해당 클래스의 필드명을 명시적으로 표현 가능
    + `this()` : 같은 클래스의 다른 생성자를 호출
---
### 12. 상속
  + 이미 개발된 클래스를 재사용하여 개발하는 방법
  + **`extends` 키워드**를 통해 상속
  + `final` 클래스는 상속 불가
  + 부모 생성자
    + 상속 받는 자식 클래스를 객체로 생성할 때, 부모 클래스의 객체가 먼저 생성된다.
    + 자식 클래스의 생성자에 부모 클래스의 생성자를 호출하는 코드가 자동으로 작성된다.
    + 명시적으로 부모 생성자를 호출하려면 `super()`를 사용한다.
  + **형변환**
    + `업캐스팅` : 자식 클래스가 부모 클래스 타입으로 캐스팅 되는 것
    + `다운캐스팅` : 부모 클래스가 자식 클래스 타입으로 캐스팅 되는 것
      + 다운 캐스팅의 진정한 의미는 부모 클래스로 업 캐스팅된 자식 클래스를 복구하여, 본인의 필드와 기능을 회복하기 위해 있는 것 
  + `Object 클래스` : 모든 클래스의 최고 조상
    + 자바 클래스가 아무것도 상속하지 않으면 `Object 클래스`를 자동으로 상속
    + 자바의 모든 인스턴스는 *`Obecjt 클래스`의 참조 변수로 참조* 가능
    + 자바의 모든 인스턴스를 대상으로 *`Object 클래스`에 정의된 메소드 호출* 가능
    + ex) 모든 인스턴스는 println()의 인자로 전달되면, toString()이 호출되어 반환되는 문자열이 출력된다. &rarr; 클래스의 적절한 정보를 반환할 수 있도록 toString()을 오버라이딩
